...
def get_upper_bound_peo_pace2017(
        old_graph, method="tamaki", wait_time=1):
    """
    ...
    from .importers import read_td_file
    from .exporters import generate_gr_file
    from .pace2017_solver_api import run_heuristic_solver
    method_args = {
        'tamaki':
        {'command': './tw-heuristic',
         'cwd': TAMAKI_SOLVER_PATH}
    }
    ...
    out_data = run_heuristic_solver(data, **method_args[method],
                                    wait_time=wait_time)
...
def get_upper_bound_peo_quickbb(
    ...
    """
    from .quickbb_api import run_quickbb
...
def get_peo(old_graph, method="tamaki"):
    ...
    from .importers import read_td_file
    from .exporters import generate_gr_file
    from .pace2017_solver_api import run_exact_solver
    ...
    data = generate_gr_file(graph)
    out_data = run_exact_solver(data, **method_args[method])
    tree, treewidth = read_td_file(out_data, as_data=True)
    peo = get_peo_from_tree(tree)
    peo = [inv_dict[pp] for pp in peo]
